    ...
    case "pr": {
      ...
      try {
        ...
        if (result.prUrl) {
          ...
          lines.push("    ⚠️  PR not created (gh CLI not available or push failed)");
          lines.push("    Branch is ready locally — push manually with:");
          lines.push(`      cd ${pathToken} && git push -u origin ${result.branchName}`);
        }

        lines.push("");
        lines.push("  PR Description (generated by LLM):");
        ...
        return `  ✗ PR generation failed: ${err instanceof Error ? err.message : String(err)}`;
      }
    }
