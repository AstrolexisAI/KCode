  ...
  switch (action) {
    ...
    case "api": {
      ...
      return [
        ...
        "  Endpoints created:",
        ...result.entities.map(
          (e) => `    /api/${e.name}s  — GET, POST, GET/:id, PUT/:id, DELETE/:id`,
        ),
        "",
    ...
    case "fullstack": {
      const desc = (args ?? "").trim();
      if (!desc)
        return "  Usage: /fullstack task management app with users\n  Creates: frontend + API + database";
    ...
    case "rust": {
      const desc = (args ?? "").trim();
      if (!desc)
        return "  Usage: /rust API server with Axum\n  /rust CLI tool for file processing\n  /rust game with Bevy";
      ...
      return [
        `  KCode Rust Engine`,
        `    ${r.config.name}/ | ${r.config.type} | ${r.files.length} files (${m} machine)`,
        "",
    ...
    case "go": {
      ...
      return [
        `  KCode Go Engine`,
        `    ${r.config.name}/ | ${r.config.type}${r.config.framework ? " (" + r.config.framework + ")" : ""} | ${r.files.length} files (${m} machine)`,
        "",
        `  make run / make test`,
      ].join("\n");
    }
    ...
    case "swift": {
      const desc = (args ?? "").trim();
      if (!desc)
        return "  Usage: /swift iOS app with SwiftUI\n  /swift macOS app\n  /swift CLI tool\n  /swift Vapor server";
    ...
    case "java": {
      const desc = (args ?? "").trim();
      if (!desc)
        return "  Usage: /java REST API with Spring\n  /java microservice with Kafka\n  /java CLI tool";
    ...
    case "docker": {
      ...
      return [
        `  KCode Docker Engine`,
        `    ${r.config.name}/ | ${r.services.length} services | ${r.files.length} files (${m} machine)`,
        "",
    ...
    case "cs": {
      const desc = (args ?? "").trim();
      if (!desc)
        return "  Usage: /csharp REST API with Entity Framework\n  /csharp Blazor app\n  /csharp CLI tool\n  /dotnet worker service";
    ...
    case "kt": {
      const desc = (args ?? "").trim();
      if (!desc)
        return "  Usage: /kotlin API with Ktor\n  /kotlin Android app with Compose\n  /kotlin CLI tool\n  /kt library";
    ...
    case "symfony": {
      const desc = (args ?? "").trim();
      if (!desc)
        return "  Usage: /php REST API with Slim\n  /php Laravel web app\n  /php CLI tool\n  /php WordPress plugin";
    ...
    case "sinatra": {
      ...
      return [
        ...
        `    ${r.config.name}/ | ${r.config.type}${r.config.framework ? " (" + r.config.framework + ")" : ""} | ${r.files.length} files (${m} machine)`,
        "",
        `  bundle exec ruby app.rb / bundle exec rspec`,
      ].join("\n");
    }
    ...
    case "phoenix": {
      const desc = (args ?? "").trim();
      if (!desc)
        return "  Usage: /elixir Phoenix API\n  /elixir LiveView app\n  /elixir CLI escript\n  /elixir GenServer worker";
    ...
    case "flutter": {
      ...
      const m = r.files.filter((f) => !f.needsLlm).length;
      return [
        `  KCode Dart/Flutter Engine`,
        `    ${r.config.name}/ | ${r.config.type}${r.config.framework ? " (" + r.config.framework + ")" : ""} | ${r.files.length} files (${m} machine)`,
        "",
        `  ${r.config.type === "mobile" || r.config.type === "web" ? "flutter run" : "dart run"} / dart test`,
      ].join("\n");
    }
    ...
    case "hs": {
      ...
      const { createHaskellProject } = await import(
        "../../core/web-engine/stacks/haskell-engine.js"
      );
      const r = createHaskellProject(desc, appConfig.workingDirectory);
      const m = r.files.filter((f) => !f.needsLlm).length;
      return [
        `  KCode Haskell Engine`,
        `    ${r.config.name}/ | ${r.config.type}${r.config.framework ? " (" + r.config.framework + ")" : ""} | ${r.files.length} files (${m} machine)`,
        "",
    ...
    case "scala": {
      const desc = (args ?? "").trim();
      if (!desc)
        return "  Usage: /scala API with http4s\n  /scala Spark data pipeline\n  /scala CLI tool";
    ...
    case "infra": {
      const desc = (args ?? "").trim();
      if (!desc)
        return "  Usage: /terraform AWS VPC with RDS and S3\n  /tf Kubernetes deployment\n  /iac GCP Cloud Run service";
      const { createTerraformProject } = await import(
        "../../core/web-engine/stacks/terraform-engine.js"
      );
      const r = createTerraformProject(desc, appConfig.workingDirectory);
      const m = r.files.filter((f) => !f.needsLlm).length;
      return [
        `  KCode Terraform Engine`,
        `    ${r.config.name}/ | ${r.config.type} | ${r.files.length} files (${m} machine)`,
        "",
    ...
    case "nx": {
      const desc = (args ?? "").trim();
      if (!desc)
        return "  Usage: /monorepo web + API with shared lib\n  /turborepo Next.js frontend with Express API\n  /nx React + Node monorepo";
      const { createMonorepoProject } = await import(
        "../../core/web-engine/stacks/monorepo-engine.js"
      );
      const r = createMonorepoProject(desc, appConfig.workingDirectory);
      const m = r.files.filter((f) => !f.needsLlm).length;
      return [
        `  KCode Monorepo Engine`,
        `    ${r.config.name}/ | ${r.config.tool} + ${r.config.packageManager} | ${r.config.packages.length} packages | ${r.files.length} files (${m} machine)`,
        "",
        `  ${r.config.packageManager} run dev / ${r.config.packageManager} run build`,
      ].join("\n");
    }
    ...
    case "pipeline": {
      ...
      return [
        `  KCode CI/CD Engine`,
        `    ${r.config.name}/ | ${r.config.platform} | ${r.config.projectType} | ${r.files.length} files (${m} machine)`,
        `    Test: ${r.config.hasTest} | Lint: ${r.config.hasLint} | Deploy: ${r.config.hasDeploy}${r.config.deployTarget ? " → " + r.config.deployTarget : ""}`,
        "",
    ...
    case "schema": {
      ...
      const m = r.files.filter((f) => !f.needsLlm).length;
      return [
        `  KCode Database Engine`,
        `    ${r.config.name}/ | ${r.config.type} + ${r.config.orm} | ${r.config.entities.length} entities | ${r.files.length} files (${m} machine)`,
        `    Entities: ${r.config.entities.map((e) => e.name).join(", ")}`,
        `    Docker: ${r.config.hasDocker ? "yes" : "no"} | Backup: ${r.config.hasBackup ? "yes" : "no"}`,
        "",
        `  make up / npm run db:init / npm run db:seed`,
      ].join("\n");
    }
    ...
    case "design-system": {
      ...
      return [
        `  KCode CSS Engine`,
        `    ${r.config.name}/ | ${r.config.type} | ${r.files.length} files (${m} machine)`,
        `    Preprocessor: ${r.config.preprocessor} | Dark mode: ${r.config.darkMode ? "yes" : "no"}`,
        "",
