...
describe("VoiceIndicator render", () => {
  ...
  test("renders nothing when idle", () => {
    instance = renderWithTheme(
      <VoiceIndicator state="idle" level={0} partialText="" />,
    );
  ...
  test("shows Calibrating state", () => {
    instance = renderWithTheme(
      <VoiceIndicator state="calibrating" level={0} partialText="" />,
    );
  ...
  test("shows Listening state with level bar", () => {
    instance = renderWithTheme(
      <VoiceIndicator state="listening" level={0.5} partialText="" />,
    );
  ...
  test("shows Transcribing state", () => {
    instance = renderWithTheme(
      <VoiceIndicator state="processing" level={0} partialText="" />,
    );
  ...
  test("shows Speaking state", () => {
    instance = renderWithTheme(
      <VoiceIndicator state="speaking" level={0} partialText="" />,
    );
  ...
  test("shows partial transcription text", () => {
    instance = renderWithTheme(
      <VoiceIndicator
        state="listening"
        level={0.3}
        partialText="Hello world this is my message"
      />,
    );
  ...
  test("truncates long partial text with ellipsis", () => {
    ...
    instance = renderWithTheme(
      <VoiceIndicator state="listening" level={0.3} partialText={long} />,
    );
  ...
  test("level 0 shows minimum bar", () => {
    instance = renderWithTheme(
      <VoiceIndicator state="listening" level={0} partialText="" />,
    );
    const out = instance.lastFrame()!;
    expect(out).toContain("Listening");
  });
  ...
  test("level 1.0 shows full bar", () => {
    instance = renderWithTheme(
      <VoiceIndicator state="listening" level={1.0} partialText="" />,
    );
    const out = instance.lastFrame()!;
