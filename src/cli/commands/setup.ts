...
function formatHardwareProfile(profile: HardwareProfile): string {
  const lines: string[] = [];
  lines.push(`  CPU:  ${profile.cpu.model} (${profile.cpu.cores} cores, ${profile.cpu.threads} threads${profile.cpu.features.length > 0 ? ", " + profile.cpu.features.join(", ").toUpperCase() : ""})`);
  ...
  lines.push(`  Disk: ${profile.storage.availableGb} GB ${profile.storage.type.toUpperCase()} available`);
  return lines.join("\n");
}

function formatRecommendation(rec: ModelRecommendation, index: number, isRecommended: boolean): string {
  const tag = isRecommended ? " [RECOMMENDED]" : "";
  const lines: string[] = [];
  lines.push(`  #${index + 1}${tag} ${rec.model}`);
  lines.push(`     VRAM: ${rec.vramRequired} GB | RAM: ${rec.ramRequired} GB | Context: ${(rec.contextWindow / 1024).toFixed(0)}K | ~${rec.estimatedTps} tok/s`);
    ...
    .action(async (opts: { model?: string; force?: boolean; list?: boolean; auto?: boolean }) => {
      ...
      if (opts.auto) {
        try {
          ...
          console.log(`  gpu_layers: ${llamaConfig.gpuLayers === -1 ? "-1 (all)" : llamaConfig.gpuLayers}`);
          ...
          console.error(`\x1b[31mHardware detection failed: ${err instanceof Error ? err.message : err}\x1b[0m`);
