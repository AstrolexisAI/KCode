...
export class McpServerConnection {
  ...
  async start(): Promise<void> {
    ...
    for (const [key, value] of Object.entries(process.env)) {
      if (value !== undefined && MCP_ENV_ALLOWLIST.has(key)) {
        filteredParentEnv[key] = value;
      }
    }
    const rawEnv = { ...filteredParentEnv, ...(this.config.env ?? {}) };
...
export class McpHttpConnection {
  ...
  private async initialize(): Promise<void> {
    ...
    await this.sendHttp("notifications/initialized", {}).catch((e) => {
      log.debug("mcp", `Failed to send initialized notification to "${this.name}": ${e}`);
    });
  }
