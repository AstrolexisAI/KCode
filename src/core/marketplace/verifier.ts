...
const KNOWN_HOOK_EVENTS = new Set([
  "pre-tool", "post-tool", "pre-commit", "post-commit",
  "pre-edit", "post-edit", "pre-write", "post-write",
  "pre-bash", "post-bash", "session-start", "session-end",
  "pre-compact", "post-compact", "pre-plan", "post-plan",
  "error", "pre-save", "post-save", "pre-send", "post-send",
  "pre-agent", "post-agent", "pre-mcp", "post-mcp",
...
export function verifyPlugin(pluginDir: string): VerificationResult {
  ...
  if (!manifest.name || typeof manifest.name !== "string") {
    issues.push({ severity: "error", code: "NO_NAME", message: "Manifest missing required 'name' field" });
  }
  if (!manifest.version || typeof manifest.version !== "string") {
    issues.push({ severity: "error", code: "NO_VERSION", message: "Manifest missing required 'version' field" });
  }
  ...
  if (Array.isArray(manifest.skills)) {
    for (const skill of manifest.skills) {
      if (typeof skill !== "string") {
        issues.push({ severity: "error", code: "INVALID_SKILL", message: `Skill entry is not a string: ${skill}` });
        continue;
      }

      // Path traversal check
      if (isPathTraversal(pluginDir, skill)) {
        issues.push({ severity: "error", code: "PATH_TRAVERSAL", message: `Skill path escapes plugin directory: ${skill}` });
        continue;
      }
  ...
  if (manifest.hooks && typeof manifest.hooks === "object" && !Array.isArray(manifest.hooks)) {
    for (const [event, hookConfig] of Object.entries(manifest.hooks as Record<string, unknown>)) {
      if (!KNOWN_HOOK_EVENTS.has(event)) {
        issues.push({ severity: "warning", code: "UNKNOWN_HOOK_EVENT", message: `Unknown hook event: ${event}` });
      }
      if (!hookConfig || typeof hookConfig !== "object") {
        issues.push({ severity: "error", code: "INVALID_HOOK", message: `Hook for "${event}" is not a valid object` });
        continue;
      }
      const hc = hookConfig as Record<string, unknown>;
      if (!hc.command || typeof hc.command !== "string") {
        issues.push({ severity: "error", code: "HOOK_NO_CMD", message: `Hook for "${event}" missing command` });
      }
    }
    ...
    for (const hook of manifest.hooks) {
      ...
      if (!h.event || typeof h.event !== "string") {
        issues.push({ severity: "error", code: "HOOK_NO_EVENT", message: "Hook entry missing event field" });
      }
      if (!h.command || typeof h.command !== "string") {
        issues.push({ severity: "error", code: "HOOK_NO_CMD", message: "Hook entry missing command field" });
      }
    }
  }
  ...
  if (manifest.mcpServers && typeof manifest.mcpServers === "object" && !Array.isArray(manifest.mcpServers)) {
    for (const [name, config] of Object.entries(manifest.mcpServers as Record<string, unknown>)) {
      if (!config || typeof config !== "object") {
        issues.push({ severity: "error", code: "MCP_INVALID", message: `MCP server "${name}" has invalid config` });
        continue;
      }
  ...
  if (Array.isArray(manifest.outputStyles)) {
    for (const style of manifest.outputStyles) {
      if (typeof style !== "string") {
        issues.push({ severity: "error", code: "INVALID_STYLE", message: `Output style entry is not a string: ${style}` });
        continue;
      }
      if (isPathTraversal(pluginDir, style)) {
        issues.push({ severity: "error", code: "PATH_TRAVERSAL", message: `Output style path escapes plugin directory: ${style}` });
        continue;
      }
      const stylePath = join(pluginDir, style);
      if (!existsSync(stylePath)) {
        issues.push({ severity: "warning", code: "MISSING_STYLE", message: `Output style file not found: ${style}` });
      }
    }
  }
  ...
  if (Array.isArray(manifest.agents)) {
    for (const agent of manifest.agents) {
      if (typeof agent !== "string") {
        issues.push({ severity: "error", code: "INVALID_AGENT", message: `Agent entry is not a string: ${agent}` });
        continue;
      }
      if (isPathTraversal(pluginDir, agent)) {
        issues.push({ severity: "error", code: "PATH_TRAVERSAL", message: `Agent path escapes plugin directory: ${agent}` });
      }
    }
  }
