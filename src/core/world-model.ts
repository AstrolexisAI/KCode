...
export class WorldModel {
  ...
  compare(prediction: Prediction, actualResult: string, isError?: boolean): void {
    try {
      ...
      ).run(prediction.action, prediction.expected, actualResult.slice(0, 500), prediction.confidence, correct ? 1 : 0);
    } catch (err) {
  ...
  private describeAction(toolName: string, input: Record<string, unknown>): string {
    switch (toolName) {
      ...
      default: return `${toolName}: ${JSON.stringify(input).slice(0, 80)}`;
    }
  }

  private estimateConfidence(toolName: string, _input: Record<string, unknown>): number {
    const baseConfidence: Record<string, number> = {
      Read: 0.9, Glob: 0.8, Grep: 0.7, Write: 0.85, Edit: 0.7, Bash: 0.6, WebFetch: 0.5, WebSearch: 0.5,
    };
  ...
  private generateExpectation(toolName: string, input: Record<string, unknown>): string {
    switch (toolName) {
      case "Read": return `File ${input.file_path} exists and is readable`;
      case "Write": return `File ${input.file_path} created/overwritten successfully`;
      case "Edit": return `Edit applied to ${input.file_path}`;
      case "Glob": return `Matching files found for pattern ${input.pattern}`;
      case "Grep": return `Pattern ${input.pattern} found in files`;
      case "Bash": {
        const cmd = String(input.command ?? "");
        if (cmd.includes("mkdir")) return "Directory created";
        if (cmd.includes("npm install") || cmd.includes("bun install")) return "Dependencies installed";
  ...
  private resultMatchesExpectation(_expected: string, actual: string): boolean {
    ...
    const errorIndicators = ["error:", "failed", "not found", "permission denied", "no such file", "enoent"];
