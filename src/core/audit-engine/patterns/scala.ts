...
export const SCALA_PATTERNS: BugPattern[] = [
  {
    ...
    verify_prompt:
      "Is user input interpolated? If using #$param (Slick) or ? placeholders, respond FALSE_POSITIVE.",
    cwe: "CWE-89",
    fix_template:
      'Use parameterized queries: sql"SELECT * FROM t WHERE id = $id" (Slick\'s safe interpolation).',
  },
  ...
  // ── Option.get ─────────────────────────────────────────────────
  {
    ...
    cwe: "CWE-476",
    fix_template:
      "Use .getOrElse(default), pattern matching, .map/.flatMap, or .fold(ifEmpty)(ifPresent).",
  },
  ...
  // ── Blocking in Future ─────────────────────────────────────────
  {
    ...
    cwe: "CWE-400",
    fix_template:
      "Wrap blocking code in scala.concurrent.blocking { }, or use a separate ExecutionContext for blocking I/O.",
  },
  ...
  // ── Mutable variable in concurrent context ─────────────────────
  {
    ...
    languages: ["scala"],
    regex:
      /\bvar\s+\w+\s*(?::\s*\w+)?\s*=[\s\S]{0,300}?\b(?:Future|Actor|Thread|Runnable|Executor|parallel)\b/g,
    explanation:
    ...
    cwe: "CWE-362",
    fix_template:
      "Use val with AtomicReference, Ref (Cats Effect), or Actor message passing instead of shared var.",
  },
  ...
  // ── Implicit conversion confusion ──────────────────────────────
  {
    ...
    cwe: "CWE-704",
    fix_template:
      "Use extension methods (Scala 3) or explicit conversion methods instead of implicit def.",
  },
  ...
  // ── Try.get without handling Failure ────────────────────────────
  {
    ...
    cwe: "CWE-755",
    fix_template:
      "Use .getOrElse(default), .recover { case e => fallback }, .toOption, or pattern matching.",
  },
  ...
  // ── Akka unhandled message ─────────────────────────────────────
  {
    ...
    cwe: "CWE-754",
    fix_template:
      'Add catch-all: case msg => log.warning(s"Unhandled message: $msg"); unhandled(msg)',
  },
  ...
  // ── SQL injection in Slick/Doobie ──────────────────────────────
  {
    ...
    cwe: "CWE-89",
    fix_template:
      'Use framework interpolation: sql"SELECT * FROM t WHERE id = ${userId}" (Slick auto-parameterizes).',
  },
  ...
  // ── Null usage in Scala ────────────────────────────────────────
  {
    ...
    languages: ["scala"],
    regex:
      /(?:\b\w+\s*(?:==|!=)\s*null\b|\b(?:val|var)\s+\w+\s*(?::\s*\w+)?\s*=\s*null\b|\breturn\s+null\b)/g,
    explanation:
    ...
    cwe: "CWE-476",
    fix_template:
      "Use Option(value) instead of null checks, None instead of null, and .map/.getOrElse for transformations.",
  },
];
