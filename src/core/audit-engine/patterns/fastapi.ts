...
export const FASTAPI_PATTERNS: BugPattern[] = [
  {
    ...
    regex:
      /\bCORSMiddleware\b[\s\S]{0,400}?allow_origins\s*=\s*\[\s*["']\*["']\s*\][\s\S]{0,400}?allow_credentials\s*=\s*True/g,
    explanation:
  ...
  },
  {
    ...
    regex:
      /\bjwt\.decode\s*\(\s*[^)]*?(?:verify\s*=\s*False|"verify_signature"\s*:\s*False|'verify_signature'\s*:\s*False)/g,
    explanation:
  ...
  },
  {
    ...
    regex:
      /\b(?:pickle|cPickle)\.loads?\s*\(\s*(?:await\s+)?\w*(?:request\.body|request\.form|request\.json|request\.read|payload|body|form_data|raw)\b/g,
    explanation:
