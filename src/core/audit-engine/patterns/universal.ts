...
export const UNIVERSAL_PATTERNS: BugPattern[] = [
  // Shell
  {
    ...
    verify_prompt:
      "Is the variable from trusted internal source or user input? CONFIRMED if user-controlled." +
  ...
  // Hardcoded IPs / URLs
  {
    ...
    verify_prompt:
      "Is this IP 127.0.0.1/localhost or 0.0.0.0 (standard)? If standard loopback, respond FALSE_POSITIVE. If it's a specific internal/production IP, respond CONFIRMED.",
    cwe: "CWE-798",
  ...
  // TODO/FIXME security markers
  {
    ...
    languages: ["python", "javascript", "typescript", "go", "java", "c", "cpp", "rust"],
    regex:
      /(?:TODO|FIXME|HACK|XXX).*(?:security|auth|password|token|secret|vuln|inject|sanitiz|escap)/gi,
    explanation:
      "A developer left a security-related TODO. This may indicate a known vulnerability that was deferred.",
    verify_prompt:
      "Is this TODO about a real security concern that hasn't been addressed? If it's already fixed (comment is stale), respond FALSE_POSITIVE.",
    cwe: "CWE-1035",
  ...
  // CWE-918: Server-Side Request Forgery (SSRF)
  {
    ...
    regex:
      /(?:requests\.(?:get|post|put|delete|patch|head)\s*\(\s*(?:f["']|[a-z_]+\s*\+|[a-z_]+\.format)|fetch\s*\(\s*(?:[a-z_]+\s*\+|`\$\{)|http\.(?:Get|Post|Do)\s*\(\s*[a-z_]|HttpClient\..*\(\s*[a-z_]|open-uri|URI\.parse\s*\(\s*(?:params|request|args))/g,
    explanation:
  ...
  // CWE-862: Missing Authorization
  {
    ...
    cwe: "CWE-862",
    fix_template:
      "Add authorization middleware/decorator: @login_required (Flask/Django), auth middleware (Express), @PreAuthorize (Spring).",
  },
  // CWE-287: Improper Authentication
  {
    ...
    // slipped through.
    regex:
      /(?:password|token|secret|api_key|apiKey|auth_token|session_id|csrf)\s*(?:===?|!==?|==|!=)\s*(?!null\b|undefined\b|None\b|["'] *["']|["']\s*\))(?:["']|[a-z_])|(?!null\b|undefined\b|None\b|["'] *["'])(?:["']|[a-z_])\w*\s*(?:===?|!==?)\s*(?:password|token|secret|api_key|apiKey|auth_token)/g,
    explanation:
  ...
  // CWE-306: Missing Authentication for Critical Function
  {
    ...
    languages: ["python", "javascript", "typescript", "go", "java", "ruby", "php"],
    regex:
      /(?:@app\.(?:route|delete|post)\s*\(\s*["'][^"']*(?:delete|remove|destroy|shutdown|reset|grant|revoke|admin|sudo|escalate|impersonate))|(?:app\.(?:delete|post)\s*\(\s*["'][^"']*(?:delete|remove|destroy|shutdown|reset|grant|revoke|admin))/g,
    explanation:
    ...
    cwe: "CWE-306",
    fix_template:
      "Add authentication + authorization middleware before the handler. Use @login_required + @admin_required or equivalent.",
  },
  // CWE-77: Command Injection (broader than CWE-78)
  {
    ...
    languages: ["python", "javascript", "typescript", "go", "java", "ruby", "php"],
    regex:
      /(?:exec\s*\(\s*["'`].*\$\{|child_process\.exec\s*\(\s*`|os\.system\s*\(\s*f["']|Runtime\.getRuntime\(\)\.exec\s*\(\s*[a-z_]+\s*\+|system\s*\(\s*["'].*#\{|Process\.Start\s*\(\s*[a-z_]+\s*\+)/g,
    explanation:
    ...
    cwe: "CWE-77",
    fix_template:
      "Use parameterized execution: subprocess.run([cmd, arg1, arg2]) instead of shell string. Never pass user input through a shell.",
  },
  // CWE-269: Improper Privilege Management
  {
    ...
    regex:
      /(?:os\.set(?:uid|gid|euid|egid)\s*\(\s*0|chmod\s+(?:777|666|a\+rwx)|setuid\s*\(\s*0\)|seteuid\s*\(\s*0\)|os\.chmod\s*\(\s*[^,]+,\s*0o?777\)|running.*as.*root|if.*os\.getuid\(\)\s*(?:!=|==)\s*0)/g,
    explanation:
    ...
    cwe: "CWE-269",
    fix_template:
      "Run with minimum required privileges. Use 0o755 instead of 0o777. Drop root after binding privileged ports.",
  },
  // CWE-94: Code Injection (broader than CWE-95 eval)
  {
    ...
    regex:
      /(?:new\s+Function\s*\(\s*[a-z_]|compile\s*\(\s*(?:[a-z_]+\s*[,)]|f["']|[a-z_]+\s*\+)|CodeDom|Roslyn.*Compile|GroovyShell|ScriptEngine.*eval|instance_eval\s*\(\s*(?:params|request|args)|create_function\s*\(\s*["']\$)/g,
    explanation:
    ...
    cwe: "CWE-94",
    fix_template:
      "Never compile user input into executable code. Use a sandboxed interpreter or a safe template engine.",
  },
  // CWE-327: Use of Broken/Risky Cryptographic Algorithm
  {
    ...
    cwe: "CWE-327",
    fix_template:
      "Replace with SHA-256+ for hashing, bcrypt/argon2 for passwords, AES-GCM for encryption, Ed25519 for signatures.",
  },
  // CWE-90: LDAP Injection
  {
    ...
    regex:
      /(?:ldap.*search.*\(\s*[^,]*\+|ldap_search\s*\([^)]*\$|DirectorySearcher.*Filter\s*=\s*[^"]*\+|LdapContext.*search\s*\([^)]*\+|ldap3.*search\s*\(\s*search_filter\s*=\s*f["'])/g,
    explanation:
    ...
    cwe: "CWE-90",
    fix_template:
      "Use parameterized LDAP queries or escape user input with ldap.filter.escape_filter_chars / LdapEncoder.filterEncode.",
  },
  // CWE-384: Session Fixation
  {
    ...
    cwe: "CWE-384",
    fix_template:
      "Call session regeneration immediately after successful authentication: req.session.regenerate() (Express), request.session.cycle_key() (Django), session_regenerate_id(true) (PHP).",
  },
  // CWE-613: Insufficient Session Expiration
  {
    ...
    languages: ["python", "javascript", "typescript", "java", "php", "ruby"],
    regex:
      /(?:session\.permanent\s*=\s*True|maxAge\s*:\s*(?:null|undefined|Infinity|[1-9][0-9]{9,})|expires\s*:\s*null|session_config.*expire.*0|cookie.*maxAge.*86400000\s*\*\s*[3-9][0-9]+)/g,
    explanation:
    ...
    cwe: "CWE-613",
    fix_template:
      "Set session expiration to 1-24 hours for sensitive apps. Use refresh token rotation for long-lived sessions.",
  },
  // CWE-59: Symlink TOCTOU (link following)
  {
    ...
    regex:
      /(?:os\.stat\s*\([^)]*\)[\s\S]{0,100}?open\s*\(|if\s+os\.path\.(?:exists|isfile)[\s\S]{0,100}?open\s*\(|access\s*\([^)]*F_OK\s*\)[\s\S]{0,100}?open\s*\(|fs\.existsSync[\s\S]{0,100}?fs\.(?:read|write)FileSync)/g,
    explanation:
    ...
    cwe: "CWE-59",
    fix_template:
      "Use atomic operations (openat with O_NOFOLLOW, fstat on the open fd, realpath + prefix check).",
  },
  // CWE-73: External Control of File Name or Path
  {
    ...
    languages: ["python", "javascript", "typescript", "go", "java", "ruby", "php"],
    regex:
      /(?:open\s*\(\s*(?:request\.|req\.|params\[|args\.|input|sys\.argv)|fs\.(?:read|write|append)File\s*\(\s*(?:req\.|request\.|params\[)|File\s*\(\s*(?:request\.|req\.|params\[)|fopen\s*\(\s*\$_(?:GET|POST|REQUEST))/g,
    explanation:
    ...
    cwe: "CWE-73",
    fix_template:
      "Use an allowlist of permitted filenames, or look up the real path from a user-supplied ID via a trusted mapping.",
  },
  // CWE-200: Information Exposure (generic)
  {
    ...
    languages: ["python", "javascript", "typescript", "go", "java", "ruby", "php"],
    regex:
      /(?:return\s+(?:jsonify|Response|res\.json)\s*\(\s*\{[^}]*(?:password|token|secret|api_key|hash|salt|private_key)|res\.send\s*\(\s*(?:err|error|exception|traceback)|DEBUG\s*=\s*True.*return)/g,
    explanation:
    ...
    cwe: "CWE-200",
    fix_template:
      "Strip sensitive fields before serializing. Use a generic error message in production and log details server-side.",
  },
  // CWE-209: Error Messages Containing Sensitive Information
  {
    ...
    regex:
      /(?:return\s+(?:str\s*\(\s*)?e(?:xception)?\s*\)|res\.(?:send|json|status\s*\(\s*500\s*\)\.send)\s*\(\s*(?:err|error|e\.stack|e\.message)|printStackTrace\s*\(\s*(?:response|resp|writer)|echo\s+\$(?:e|exception)|raise\s+HTTPException\s*\([^)]*str\s*\(\s*e)/g,
    explanation:
    ...
    cwe: "CWE-209",
    fix_template:
      "In production, return a generic 'Internal server error' and log the exception server-side with a correlation ID the user can reference.",
  },
  // CWE-863: Incorrect Authorization
  {
    ...
    languages: ["python", "javascript", "typescript", "go", "java", "ruby", "php"],
    regex:
      /(?:(?:is_admin|isAdmin|is_superuser|isSuperuser|role|permission)\s*=\s*(?:request\.|req\.|params\[|args\.|body\.|query\.))|(?:if\s*\(\s*(?:req|request)\.(?:body|query|params|cookies)\s*\.\s*(?:admin|role|permission|is_admin|isAdmin|is_superuser))/g,
    explanation:
    ...
    cwe: "CWE-863",
    fix_template:
      "Read authorization data from the server session or a validated JWT — never from request body/query/cookies.",
  },
];
