...
describe("CostTracker", () => {
  ...
  test("getSummary byModel groups correctly", () => {
    const now = Date.now();
    tracker.record({ timestamp: now, sessionId: "s1", model: "model-a", provider: "p", inputTokens: 100, outputTokens: 50, costUsd: 0.01 });
    tracker.record({ timestamp: now, sessionId: "s1", model: "model-a", provider: "p", inputTokens: 200, outputTokens: 100, costUsd: 0.02 });
    tracker.record({ timestamp: now, sessionId: "s1", model: "model-b", provider: "p", inputTokens: 50, outputTokens: 25, costUsd: 0.005 });
