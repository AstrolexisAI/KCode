...
export type CicdProjectType = "node" | "python" | "go" | "rust" | "java" | "dotnet" | "docker" | "terraform" | "generic";
...
export interface CicdProjectResult { config: CicdConfig; files: GenFile[]; projectPath: string; prompt: string; }

function buildSteps(cfg: CicdConfig): { setup: string; test: string; lint: string; build: string } {
  switch (cfg.projectType) {
    case "node": return { setup: "uses: actions/setup-node@v4\n        with: { node-version: 22 }\n      - run: npm ci", test: "npm test", lint: "npm run lint", build: "npm run build" };
    case "python": return { setup: "uses: actions/setup-python@v5\n        with: { python-version: '3.13' }\n      - run: pip install -e '.[dev]'", test: "pytest", lint: "ruff check .", build: "python -m build" };
    case "go": return { setup: "uses: actions/setup-go@v5\n        with: { go-version: '1.23' }", test: "go test ./...", lint: "golangci-lint run", build: "go build ./..." };
    case "rust": return { setup: "uses: dtolnay/rust-toolchain@stable\n      - uses: Swatinem/rust-cache@v2", test: "cargo test", lint: "cargo clippy -- -D warnings", build: "cargo build --release" };
    case "java": return { setup: "uses: actions/setup-java@v4\n        with: { distribution: temurin, java-version: 21 }", test: "./gradlew test", lint: "./gradlew check", build: "./gradlew build" };
    case "dotnet": return { setup: "uses: actions/setup-dotnet@v4\n        with: { dotnet-version: '9.0' }", test: "dotnet test", lint: "dotnet format --verify-no-changes", build: "dotnet build -c Release" };
    case "terraform": return { setup: "uses: hashicorp/setup-terraform@v3", test: "terraform validate", lint: "terraform fmt -check", build: "terraform plan" };
    case "docker": return { setup: "uses: docker/setup-buildx-action@v3", test: "docker compose run --rm app test", lint: "hadolint Dockerfile", build: "docker build -t ${{ github.repository }}:${{ github.sha }} ." };
    default: return { setup: "run: echo 'Setup'", test: "echo 'Tests'", lint: "echo 'Lint'", build: "echo 'Build'" };
  }
}
...
export function createCicdProject(userRequest: string, cwd: string): CicdProjectResult {
  ...
  if (cfg.platform === "github") {
    ...
    gitlabCi += `build:\n  stage: build\n  script:\n    - ${steps.build}\n  artifacts:\n    paths:\n      - dist/\n\n`;
    if (cfg.hasDeploy) gitlabCi += `deploy:\n  stage: deploy\n  script:\n    - echo "TODO: deploy"\n  only:\n    - main\n  when: manual\n`;
    ...
    if (cfg.hasLint) jenkinsfile += `    stage('Lint') {\n      steps {\n        sh '${steps.lint}'\n      }\n    }\n`;
    if (cfg.hasTest) jenkinsfile += `    stage('Test') {\n      steps {\n        sh '${steps.test}'\n      }\n    }\n`;
    jenkinsfile += `    stage('Build') {\n      steps {\n        sh '${steps.build}'\n      }\n    }\n`;
    if (cfg.hasDeploy) jenkinsfile += `    stage('Deploy') {\n      when { branch 'main' }\n      steps {\n        sh 'echo TODO: deploy'\n      }\n    }\n`;
    jenkinsfile += `  }\n\n  post {\n    always {\n      cleanWs()\n    }\n  }\n}\n`;
  ...
  return { config: cfg, files, projectPath, prompt: `CI/CD pipeline (${cfg.platform}) for ${cfg.projectType}. ${m} files machine. USER: "${userRequest}"` };
}
