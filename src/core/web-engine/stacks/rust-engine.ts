...
function detectRustProject(msg: string): RustConfig {
  ...
  if (/\b(?:api|server|http|web|rest|actix|axum|rocket)\b/i.test(lower)) {
    type = "api";
    if (/\bactix\b/i.test(lower)) {
      deps.push({ name: "actix-web", version: "4" }, { name: "actix-rt", version: "2" });
    } else {
      deps.push(
    ...
    deps.push(
      ...
      { name: "tower-http", version: "0.6", features: ["cors"] },
      { name: "tracing", version: "0.1" },
      { name: "tracing-subscriber", version: "0.3", features: ["env-filter"] },
      { name: "dotenvy", version: "0.15" },
    );
  } else if (/\b(?:lib|library|crate)\b/i.test(lower)) {
    type = "library";
  } else if (/\b(?:wasm|web\s*assembly|browser)\b/i.test(lower)) {
    type = "wasm";
    deps.push({ name: "wasm-bindgen", version: "0.2" });
  } else if (/\b(?:embedded|no_std|microcontroller|stm32|esp)\b/i.test(lower)) {
    type = "embedded";
  } else if (/\b(?:game|bevy|ggez)\b/i.test(lower)) {
    type = "game";
    deps.push({ name: "bevy", version: "0.15" });
  } else if (/\b(?:system|daemon|service)\b/i.test(lower)) {
    type = "system";
    deps.push({ name: "tokio", version: "1", features: ["full"] });
  } else {
    deps.push(
      { name: "clap", version: "4", features: ["derive"] },
      { name: "anyhow", version: "1" },
    );
  }

  if (/\b(?:async|tokio)\b/i.test(lower) && !deps.some((d) => d.name === "tokio"))
    deps.push({ name: "tokio", version: "1", features: ["full"] });
  if (/\b(?:serde|json|serialize)\b/i.test(lower) && !deps.some((d) => d.name === "serde"))
    deps.push(
      { name: "serde", version: "1", features: ["derive"] },
      { name: "serde_json", version: "1" },
    );
  if (/\b(?:sql|sqlite|postgres|database|db)\b/i.test(lower))
    deps.push({ name: "sqlx", version: "0.8", features: ["runtime-tokio", "sqlite"] });
  if (/\b(?:log|tracing)\b/i.test(lower))
    deps.push({ name: "tracing", version: "0.1" }, { name: "tracing-subscriber", version: "0.3" });
  if (/\b(?:reqwest|http\s*client|fetch)\b/i.test(lower))
    deps.push({ name: "reqwest", version: "0.12", features: ["json"] });
  ...
  return deps
    .map((d) => {
      if (d.features?.length)
        return `${d.name} = { version = "${d.version}", features = [${d.features.map((f) => `"${f}"`).join(", ")}] }`;
...
`,
  ...
  files.push({
    path: "Dockerfile",
    content: `FROM rust:1.82 AS builder
WORKDIR /app
COPY . .
...
`,
  ...
  return {
    ...
    files,
    projectPath,
    prompt: `Implement a Rust ${cfg.type}. ${m} files created by machine, implement TODO in ${l} files. USER: "${userRequest}"`,
  };
}
