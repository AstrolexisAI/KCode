...
export type DartProjectType = "mobile" | "web" | "cli" | "library" | "server" | "package" | "custom";

interface DartConfig { name: string; type: DartProjectType; framework?: string; deps: Array<{ name: string; version: string }>; }
...
function detectDartProject(msg: string): DartConfig {
  ...
  if (/\b(?:flutter\s*web|web\s*app|webapp|pwa)\b/i.test(lower)) {
    type = "web"; framework = "flutter";
  }
  else if (/\b(?:mobile|flutter|ios|android|app)\b/i.test(lower)) {
    type = "mobile"; framework = "flutter";
  }
  else if (/\b(?:server|backend|api|rest|shelf|dart.?frog)\b/i.test(lower)) {
    type = "server";
    if (/\b(?:dart.?frog)\b/i.test(lower)) { framework = "dart_frog"; deps.push({ name: "dart_frog", version: "^1.1.0" }); }
    else { framework = "shelf"; deps.push({ name: "shelf", version: "^1.4.0" }, { name: "shelf_router", version: "^1.1.0" }); }
  }
  else if (/\b(?:cli|command|terminal|tool|console)\b/i.test(lower)) { type = "cli"; deps.push({ name: "args", version: "^2.5.0" }); }
  else if (/\b(?:lib|library)\b/i.test(lower)) { type = "library"; }
  else if (/\b(?:package|pub)\b/i.test(lower)) { type = "package"; }
  ...
  if (/\b(?:provider)\b/i.test(lower) && !deps.some(d => d.name.includes("riverpod"))) deps.push({ name: "provider", version: "^6.1.0" });

  // Networking
  if (/\b(?:dio)\b/i.test(lower)) deps.push({ name: "dio", version: "^5.4.0" });
  if (/\b(?:http\b)/i.test(lower) && !deps.some(d => d.name === "dio")) deps.push({ name: "http", version: "^1.2.0" });

  // Database
  if (/\b(?:hive)\b/i.test(lower)) deps.push({ name: "hive", version: "^2.2.0" }, { name: "hive_flutter", version: "^1.1.0" });
  ...
  // Firebase
  if (/\b(?:firebase)\b/i.test(lower)) deps.push({ name: "firebase_core", version: "^2.27.0" }, { name: "firebase_auth", version: "^4.17.0" });
...
export interface DartProjectResult { config: DartConfig; files: GenFile[]; projectPath: string; prompt: string; }
...
export function createDartProject(userRequest: string, cwd: string): DartProjectResult {
  ...
  files.push({ path: "pubspec.yaml", content: `name: ${cfg.name}
...
${isFlutter ? "\nflutter:\n  uses-material-design: true\n" : ""}`, needsLlm: false });
  ...
  files.push({ path: "analysis_options.yaml", content: `include: package:lints/recommended.yaml
...
`, needsLlm: false });
    ...
    files.push({ path: "lib/app.dart", content: `import 'package:flutter/material.dart';
...
`, needsLlm: true });
    ...
    files.push({ path: "lib/screens/home_screen.dart", content: `import 'package:flutter/material.dart';
...
`, needsLlm: true });
    ...
    files.push({ path: "web/index.html", content: `<!DOCTYPE html>
<html>
<head>
  <base href="/">
  <meta charset="UTF-8">
...
`, needsLlm: false });
    ...
    files.push({ path: "bin/main.dart", content: `import 'package:args/args.dart';
...
`, needsLlm: true });
    ...
    files.push({ path: "routes/index.dart", content: `import 'package:dart_frog/dart_frog.dart';
...
`, needsLlm: true });
    ...
    files.push({ path: "routes/health.dart", content: `import 'package:dart_frog/dart_frog.dart';
...
`, needsLlm: false });
    ...
    files.push({ path: "main.dart", content: `import 'dart:io';
...
`, needsLlm: false });
    ...
    files.push({ path: "bin/server.dart", content: `import 'dart:io';
...
`, needsLlm: true });
    ...
    files.push({ path: `lib/src/${cfg.name}.dart`, content: `/// ${cap(cfg.name)} — Main library class.
class ${cap(cfg.name)} {
...
`, needsLlm: true });
    ...
    files.push({ path: `lib/${cfg.name}.dart`, content: `/// ${cap(cfg.name)} library.
library;
...
`, needsLlm: false });
    ...
    files.push({ path: "lib/main.dart", content: `void main() {
...
`, needsLlm: true });
    ...
    files.push({ path: `test/${cfg.name}_test.dart`, content: `import 'package:test/test.dart';
...
`, needsLlm: true });
  ...
  // Extras
  files.push({ path: ".gitignore", content: `.dart_tool/
.packages
build/
.pub/
.env
...
`, needsLlm: false });
  ...
  files.push({ path: "Dockerfile", content: `FROM dart:stable AS builder
WORKDIR /app
COPY pubspec.* ./
RUN dart pub get
COPY . .
...
`, needsLlm: false });
  ...
  return { config: cfg, files, projectPath, prompt: `Implement Dart ${cfg.type}${cfg.framework ? " (" + cfg.framework + ")" : ""}. ${m} files machine. USER: "${userRequest}"` };
}
