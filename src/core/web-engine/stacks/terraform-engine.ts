...
export type TerraformProjectType =
  | "aws"
  | "gcp"
  | "azure"
  | "kubernetes"
...
export interface TerraformProjectResult {
...
function detectProjectType(msg: string): TerraformProjectType {
  ...
  if (
    /\b(?:docker|container(?:s)?)\b/i.test(lower) &&
    !/\b(?:ecs|ecr|fargate|aws|gcp|azure)\b/i.test(lower)
  )
    return "docker";
...
function detectComponents(msg: string): InfraComponent[] {
  ...
  if (/\b(?:ec2|instance|compute|vm|server)\b/i.test(lower))
    components.push({ name: "compute", kind: "compute" });
  if (/\b(?:rds|database|db|aurora|mysql|postgres)\b/i.test(lower))
    components.push({ name: "database", kind: "database" });
  if (/\b(?:s3|bucket|storage|blob)\b/i.test(lower))
    components.push({ name: "storage", kind: "storage" });
  if (/\b(?:lambda|function|serverless|cloud\s*function)\b/i.test(lower))
    components.push({ name: "serverless", kind: "serverless" });
  if (/\b(?:ecs|fargate|container|docker)\b/i.test(lower))
    components.push({ name: "container", kind: "container" });
  if (/\b(?:eks|gke|aks|k8s|kubernetes)\b/i.test(lower))
    components.push({ name: "k8s", kind: "k8s" });
  if (/\b(?:cloudfront|cdn|distribution)\b/i.test(lower))
    components.push({ name: "cdn", kind: "cdn" });
  if (/\b(?:route53|dns|domain|hosted\s*zone)\b/i.test(lower))
    components.push({ name: "dns", kind: "dns" });
  if (/\b(?:iam|role|policy|security|identity)\b/i.test(lower))
    components.push({ name: "security", kind: "security" });
...
export function createTerraformProject(userRequest: string, cwd: string): TerraformProjectResult {
  ...
  files.push({
    ...
    content: buildTfvarsExample(cfg),
    needsLlm: false,
  });
  ...
  if (cfg.hasModules) {
    if (
      cfg.components.find((c) => c.kind === "vpc") &&
      (cfg.type === "aws" || cfg.type === "multi-cloud")
    ) {
  ...
  return {
    ...
    files,
    projectPath,
    prompt: `Terraform ${cfg.type} project with ${cfg.components.length} components. ${m} files machine. USER: "${userRequest}"`,
  };
}
