...
function detectMonorepoProject(msg: string): MonorepoConfig {
  ...
  if (/\b(?:web|frontend|next|react|vue|svelte)\b/i.test(lower))
    packages.push({ name: "web", type: "web" });
  if (/\b(?:api|backend|server|express|fastify)\b/i.test(lower))
    packages.push({ name: "api", type: "api" });
...
export interface MonorepoProjectResult {
...
export function createMonorepoProject(userRequest: string, cwd: string): MonorepoProjectResult {
  ...
  files.push({
    ...
    content: `{ "extends": "${pkgPrefix}/tsconfig/base.json", "compilerOptions": { "outDir": "dist", "rootDir": "src" }, "include": ["src"] }\n`,
    needsLlm: false,
  });
  files.push({
  ...
  for (const pkg of cfg.packages) {
    ...
    files.push({
      path: `${dir}/${pkg.name}/package.json`,
      content: JSON.stringify(
        {
          name: `${pkgPrefix}/${pkg.name}`,
          version: "0.1.0",
          type: "module",
          scripts: {
            dev:
    ...
    files.push({
      path: `${dir}/${pkg.name}/tsconfig.json`,
      content: `{ "extends": "${pkgPrefix}/tsconfig/base.json", "compilerOptions": { "outDir": "dist", "rootDir": "src" }, "include": ["src"] }\n`,
      needsLlm: false,
    });
    ...
    if (pkg.type === "web") {
      ...
      files.push({
        path: `${dir}/${pkg.name}/src/index.test.ts`,
      ...
      files.push({
        path: `${dir}/${pkg.name}/src/Button.ts`,
        content: `export interface ButtonProps { label: string; onClick?: () => void; variant?: "primary" | "secondary"; }\n\n// TODO: implement component\n`,
        needsLlm: true,
      });
    } else {
      files.push({
  ...
  files.push({
    path: ".prettierrc",
    content: `{ "semi": true, "singleQuote": false, "tabWidth": 2, "trailingComma": "all" }\n`,
    needsLlm: false,
  });
  files.push({
    path: "README.md",
  ...
  return {
    ...
    files,
    projectPath,
    prompt: `Monorepo (${cfg.tool}). ${cfg.packages.length} packages. ${m} files machine. USER: "${userRequest}"`,
  };
}
