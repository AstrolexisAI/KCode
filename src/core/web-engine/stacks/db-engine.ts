...
function detectDbProject(msg: string): DbConfig {
  ...
  // DB type
  if (/\b(?:mssql|sql\s*server|sqlserver|microsoft\s*sql)\b/i.test(lower)) {
    type = "mssql";
    port = 1433;
  } else if (/\b(?:mysql|mariadb)\b/i.test(lower)) {
    type = "mysql";
    port = 3306;
  } else if (/\b(?:sqlite|lite)\b/i.test(lower)) {
    type = "sqlite";
    port = 0;
  } else if (/\b(?:mongo|mongodb|nosql)\b/i.test(lower)) {
    type = "mongo";
    port = 27017;
    orm = "mongoose";
  } else if (/\b(?:redis)\b/i.test(lower)) {
    type = "redis";
    port = 6379;
    orm = "none";
  } else if (/\b(?:supabase)\b/i.test(lower)) {
    type = "supabase";
    port = 5432;
    orm = "raw";
  }
...
function genCreateSQL(entities: Entity[], type: DbProjectType): string {
  ...
  for (const e of entities) {
    ...
    for (const f of e.fields) {
      ...
      if (f.name === "slug" || f.name === "email")
        lines.push(`${idxPrefix} idx_${e.table}_${f.name} ON ${e.table}(${f.name});`);
    }
    if (e.fields.find((f) => f.name === "created_at"))
      lines.push(`${idxPrefix} idx_${e.table}_created_at ON ${e.table}(created_at DESC);`);
    lines.push("");
  }
  ...
  if (type === "postgres") {
    ...
    for (const e of entities) {
      if (e.fields.find((f) => f.name === "updated_at")) {
        lines.push(
          `CREATE TRIGGER trg_${e.table}_updated_at BEFORE UPDATE ON ${e.table} FOR EACH ROW EXECUTE FUNCTION update_updated_at();`,
        );
      }
    }
  }
...
function genSeedSQL(entities: Entity[]): string {
  ...
  for (const e of entities) {
    if (e.name === "user") {
      ...
      lines.push(
        `  ('Hello World', 'hello-world', 'First post content.', true, (SELECT id FROM users LIMIT 1));`,
      );
    } else if (e.name === "category") {
      lines.push(
        `INSERT INTO categories (name, slug) VALUES ('General', 'general'), ('Tech', 'tech');`,
      );
    } else if (e.name === "tag") {
      lines.push(
        `INSERT INTO tags (name, slug) VALUES ('javascript', 'javascript'), ('typescript', 'typescript');`,
      );
    }
    lines.push("");
  }
  ...
  for (const e of entities) {
    ...
    for (const f of e.fields) {
      ...
      if (f.type === "uuid") col += `uuid("${f.name}")`;
        ...
        col += `varchar("${f.name}", { length: ${f.type.match(/\d+/)?.[0] ?? 255} })`;
        ...
        col += `decimal("${f.name}", { precision: 10, scale: 2 })`;
...
function genMongooseSchemas(entities: Entity[]): string {
  ...
  for (const e of entities) {
    ...
    for (const f of e.fields) {
      ...
      if (!f.nullable && f.name !== "created_at" && f.name !== "updated_at")
        field += ", required: true";
      ...
      if (f.ref)
        field += `, ref: "${f.ref.split("(")[0]!.charAt(0).toUpperCase() + f.ref.split("(")[0]!.slice(1, -1)}"`;
      field += " },";
      lines.push(field);
    }
...
export function createDbProject(userRequest: string, cwd: string): DbProjectResult {
  ...
  files.push({
    path: "sql/schema.sql",
    content: genCreateSQL(cfg.entities, cfg.type),
    needsLlm: false,
  });
  if (cfg.hasSeed) {
  ...
  if (cfg.orm === "prisma") {
    files.push({
      ...
      content: genPrismaSchema(cfg.entities, cfg.type),
      needsLlm: false,
    });
  } else if (cfg.orm === "drizzle") {
    files.push({
      path: "src/db/schema.ts",
      content: genDrizzleSchema(cfg.entities),
      needsLlm: false,
    });
    files.push({
    ...
    files.push({
      ...
      content: genMongooseSchemas(cfg.entities),
      needsLlm: false,
    });
    files.push({
    ...
    for (const e of cfg.entities) {
      const modelName = e.name.charAt(0).toUpperCase() + e.name.slice(1);
      files.push({
        path: `src/entities/${modelName}.ts`,
        content: `import { Entity, PrimaryGeneratedColumn, Column, CreateDateColumn, UpdateDateColumn } from "typeorm";\n\n@Entity("${e.table}")\nexport class ${modelName} {\n${e.fields
          .map((f) => {
            ...
            const tsType =
              f.type.startsWith("varchar") || f.type === "text" || f.type === "uuid"
                ? "string"
                : f.type === "boolean"
                  ? "boolean"
                  : f.type === "integer"
                    ? "number"
                    : f.type.startsWith("decimal")
                      ? "number"
                      : "string";
  ...
  if (cfg.orm !== "mongoose" && cfg.orm !== "none") {
    const connUrl =
      cfg.type === "sqlite"
        ? `./dev.db`
        : `${cfg.type === "postgres" ? "postgresql" : cfg.type}://app:changeme@localhost:${cfg.port}/${cfg.name}`;
    files.push({
  ...
  if (cfg.orm === "typeorm") {
    ...
    if (cfg.type === "postgres") deps["pg"] = "*";
  }
  if (cfg.orm === "knex") {
    deps["knex"] = "*";
    if (cfg.type === "postgres") deps["pg"] = "*";
    if (cfg.type === "mysql") deps["mysql2"] = "*";
    if (cfg.type === "sqlite") deps["better-sqlite3"] = "*";
  }
  if (cfg.orm === "mongoose") {
    deps["mongoose"] = "*";
  }
    ...
    content: JSON.stringify(
      {
        ...
        scripts: {
          ...
          "db:init":
            cfg.type === "sqlite"
              ? "sqlite3 dev.db < sql/schema.sql"
              : `psql $DATABASE_URL < sql/schema.sql`,
          "db:seed": cfg.hasSeed
            ? cfg.type === "sqlite"
              ? "sqlite3 dev.db < sql/seed.sql"
              : `psql $DATABASE_URL < sql/seed.sql`
            : "echo 'no seed'",
        },
        dependencies: deps,
  ...
  if (cfg.type === "postgres")
    envLines.push(`DATABASE_URL=postgresql://app:changeme@localhost:5432/${cfg.name}`);
  else if (cfg.type === "mysql")
    envLines.push(`DATABASE_URL=mysql://app:changeme@localhost:3306/${cfg.name}`);
  else if (cfg.type === "sqlite") envLines.push(`DATABASE_URL=file:./dev.db`);
  else if (cfg.type === "mssql")
    envLines.push(`DATABASE_URL=mssql://sa:Changeme1!@localhost:1433/${cfg.name}`);
  else if (cfg.type === "mongo")
    envLines.push(`MONGO_URI=mongodb://app:changeme@localhost:27017/${cfg.name}`);
  else if (cfg.type === "redis") envLines.push(`REDIS_URL=redis://:changeme@localhost:6379`);

  files.push({ path: ".env", content: envLines.join("\n") + "\n", needsLlm: false });
  files.push({
    path: ".env.example",
    content: envLines.map((l) => l.replace("changeme", "YOUR_PASSWORD")).join("\n") + "\n",
    needsLlm: false,
  });
  ...
  return {
    ...
    files,
    projectPath,
    prompt: `${cfg.type} database with ${cfg.orm}. Entities: ${cfg.entities.map((e) => e.name).join(", ")}. ${m} files machine. USER: "${userRequest}"`,
  };
}
