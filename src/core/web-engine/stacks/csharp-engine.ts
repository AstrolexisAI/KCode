...
interface CSharpConfig {
  name: string;
  type: CSharpProjectType;
  framework?: string;
  deps: string[];
...
function detectCSharpProject(msg: string): CSharpConfig {
  ...
  if (/\b(?:blazor|wasm|webassembly|spa)\b/i.test(lower)) {
    type = "blazor";
    ...
    type = "cli";
    deps.push("System.CommandLine");
  } else if (/\b(?:lib|library|nuget|package)\b/i.test(lower)) {
    type = "library";
  } else if (/\b(?:worker|background|service|queue|hosted)\b/i.test(lower)) {
    type = "worker";
  } else if (/\b(?:grpc|protobuf)\b/i.test(lower)) {
    type = "grpc";
  } else if (/\b(?:maui|mobile|xamarin|ios|android|cross.?platform)\b/i.test(lower)) {
    type = "maui";
  } else if (/\b(?:game|unity|monogame|godot)\b/i.test(lower)) {
    type = "game";
    framework = "monogame";
  }

  if (/\b(?:ef|entity\s*framework|database|db|postgres|sql)\b/i.test(lower))
    deps.push("Microsoft.EntityFrameworkCore", "Microsoft.EntityFrameworkCore.Sqlite");
  if (/\b(?:swagger|openapi)\b/i.test(lower)) deps.push("Swashbuckle.AspNetCore");
  if (/\b(?:auth|identity|jwt)\b/i.test(lower))
    deps.push("Microsoft.AspNetCore.Authentication.JwtBearer");
...
export interface CSharpProjectResult {
  config: CSharpConfig;
  files: GenFile[];
  projectPath: string;
  prompt: string;
...
export function createCSharpProject(userRequest: string, cwd: string): CSharpProjectResult {
  ...
  // .csproj
  const pkgRefs = cfg.deps
    .map((d) => `    <PackageReference Include="${d}" Version="*" />`)
    .join("\n");
  const sdkType =
    cfg.type === "blazor"
      ? "Microsoft.NET.Sdk.BlazorWebAssembly"
      : ["api", "grpc"].includes(cfg.type)
        ? "Microsoft.NET.Sdk.Web"
        : "Microsoft.NET.Sdk";

  files.push({
    path: `${cfg.name}.csproj`,
    content: `<Project Sdk="${sdkType}">
...
`,
      ...
      files.push({
        path: "Program.cs",
        content: `using ${cfg.name}.Middleware;
...
`,
      ...
      files.push({
        path: "Models/Item.cs",
        content: `namespace ${cfg.name}.Models;
...
`,
      ...
      files.push({
        path: "Middleware/GlobalExceptionHandler.cs",
        content: `namespace ${cfg.name}.Middleware;
...
`,
    ...
    files.push({
      path: `Tests/${cfg.name}Tests.cs`,
      content: `namespace ${cfg.name}.Tests;
...
`,
  ...
  const testExtraPkgs =
    cfg.type === "api"
      ...
      : "";
  files.push({
    path: `Tests/${cfg.name}.Tests.csproj`,
...
`,
  ...
  files.push({
    path: "Dockerfile",
    content: `FROM mcr.microsoft.com/dotnet/sdk:9.0 AS build
WORKDIR /src
COPY *.csproj ./
RUN dotnet restore
COPY . .
...
`,
  ...
  return {
    ...
    files,
    projectPath,
    prompt: `Implement C#/.NET ${cfg.type}${cfg.framework ? " (" + cfg.framework + ")" : ""}. ${m} files machine. USER: "${userRequest}"`,
  };
}
