...
export type DebugCategory = "decision" | "routing" | "tool" | "context" | "permission" | "guard" | "hook" | "model";
...
export class DebugTracer {
  ...
  trace(category: DebugCategory, action: string, reason: string, details?: Record<string, unknown>): void {
  ...
  formatTrace(events?: DebugEvent[]): string {
    ...
    const lines = [
      `  Debug Trace (${evts.length} events)`,
      `  ${"─".repeat(60)}`,
    ];
  ...
  traceToolChoice(toolName: string, reason: string, alternatives?: string[]): void {
    if (!this.enabled) return;
    this.trace("tool", `Selected tool: ${toolName}`, reason, alternatives ? { alternatives } : undefined);
  }
  ...
  tracePermission(tool: string, action: string, rule?: string): void {
    if (!this.enabled) return;
    this.trace("permission", `${tool}: ${action}`, rule ?? "default policy", rule ? { rule } : undefined);
  }
  ...
  traceCompaction(tokensBefore: number, tokensAfter: number, method: string): void {
    ...
    this.trace("context", `Compaction (${method})`, `Reduced ${tokensBefore} -> ${tokensAfter} tokens (saved ${saved})`, {
      tokensBefore,
      tokensAfter,
      saved,
      method,
  ...
  traceGuard(guard: string, triggered: boolean, details?: string): void {
    if (!this.enabled) return;
    const action = triggered ? `Guard triggered: ${guard}` : `Guard checked: ${guard} (not triggered)`;
  ...
  traceRouting(taskType: string, selectedModel: string, candidates?: string[]): void {
    if (!this.enabled) return;
    this.trace("routing", `Route: ${taskType} -> ${selectedModel}`, `Task classified as "${taskType}"`, {
      taskType,
      selectedModel,
