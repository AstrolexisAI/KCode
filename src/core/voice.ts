...
function recordAudio(durationSec: number): string {
  ...
  try {
    execSync(
      `sox -d -r ${SAMPLE_RATE} -c 1 -b 16 "${outPath}" trim 0 ${durationSec} 2>/dev/null`,
      { stdio: "pipe", timeout: (durationSec + 2) * 1000 },
    );
    return outPath;
 ...
 */
export function isVoiceAvailable(): { available: boolean; backends: VoiceBackend[]; recording: boolean } {
