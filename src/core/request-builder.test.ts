...
import { resolveApiKey, estimateToolDefinitionTokens } from "./request-builder.ts";
...
describe("resolveApiKey", () => {
  ...
  test("URL with together.xyz resolves TOGETHER_API_KEY", () => {
    ...
    expect(resolveApiKey("llama-3", "https://api.together.xyz/v1", baseConfig)).toBe("together-key");
  });

  test("unknown model falls back to config.apiKey", () => {
    expect(resolveApiKey("custom-model", "http://localhost:10091", baseConfig)).toBe("fallback-key");
  });
...
describe("estimateToolDefinitionTokens", () => {
  ...
  function createMockRegistry(defs: Array<{ name: string; description: string; input_schema: Record<string, unknown> }>) {
    return {
      getDefinitions: () => defs,
    } as any;
  }
  ...
  test("tools with complex schemas produce larger estimates", () => {
    ...
    const complexRegistry = createMockRegistry([
      {
        name: "Complex", description: "Complex tool with many parameters",
        input_schema: {
          type: "object",
          properties: {
    ...
    expect(estimateToolDefinitionTokens(complexRegistry)).toBeGreaterThan(estimateToolDefinitionTokens(simpleRegistry));
  });

  test("estimate is roughly proportional to JSON size / 3.5", () => {
    const defs = [
      { name: "TestTool", description: "A test tool for estimation", input_schema: { type: "object", properties: { a: { type: "string" } } } },
    ];
    ...
    const expectedChars = defs[0]!.name.length + defs[0]!.description.length + JSON.stringify(defs[0]!.input_schema).length + 50;
