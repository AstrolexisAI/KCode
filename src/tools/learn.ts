...
You can also use this tool to search, update, or delete existing learnings.`,
  input_schema: {
    ...
    properties: {
      ...
      topic: {
        type: "string",
        description: "Short topic name (e.g., 'next-js-gotchas', 'user prefers typescript', 'port-conflicts')",
      },
      content: {
        type: "string",
      ...
      scope: {
        type: "string",
        enum: ["global", "project"],
        description: "Scope: 'global' for general learnings, 'project' for project-specific. Default: global.",
      },
      tags: {
        type: "string",
        description: "Comma-separated tags for categorization (e.g., 'typescript,convention,testing')",
      },
      query: {
        type: "string",
...
export function loadLearnings(projectPath?: string, contextKeywords?: string[]): string | null {
  try {
    ...
    if (projectPath) {
      db.query("UPDATE learnings SET access_count = access_count + 1 WHERE scope = 'project' AND project = ?").run(projectPath);
    }
  ...
  try {
    ...
    switch (action) {
      ...
      case "search": {
        ...
        ).all(query) as { id: number; topic: string; content: string; scope: string; tags: string; updated_at: string }[];
      ...
      case "list": {
        ...
        ).all() as { id: number; topic: string; scope: string; tags: string; updated_at: string; access_count: number }[];
      ...
      case "delete": {
        ...
        const row = db.query("SELECT topic FROM learnings WHERE id = ?").get(id) as { topic: string } | null;
