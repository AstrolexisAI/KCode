...
2. Show HN: KCode – deterministic security scanner for C/Rust/Go, 441 patterns, local-first
...
KCode is a static security scanner that flips the usual LLM-SAST split. Instead of sending your whole codebase to a cloud model and hoping it notices the bug, a deterministic scanner runs 441 hand-written patterns locally and then a small local LLM (runs on a 24GB GPU) verifies each candidate in isolation. The LLM's job is just to strip false positives — not to find bugs.
